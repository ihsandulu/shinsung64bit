...
<div id="area_print">
    <div style="overflow-x:auto;">
        ...
        foreach ($output1 as $row1) {
            ...
            //SQL UNitname
            $sql = $this->db->from("inspect_v2 AS i")
                ->select("i.line, i.kanaan_po, i.style, COUNT(DISTINCT i.unit_name)AS jmlunit")
                ->join("daftar_defect AS d", "d.kode=i.kode_defect", "left")
                ->where("convert(date, i.tanggal)>='" . $from . "'")
                ->where("convert(date, i.tanggal)<='" . $to . "'")
            ...
            $urun = array();
            foreach ($output as $row) {
                $urun[$row['line']][$row['kanaan_po']][$row['style']] = isset($row["jmlunit"]) ? $row["jmlunit"] : 0;
            }
            // pre($urun);
                ...
                <tbody>
                    ...
                    ?>
                            <tr>
                                ...
                                <?php 
                                    $garmentur = $urun[$value->line][$value->kanaan_po][$value->style];  
                                    echo $garmentur;                                        
                                ?>
                                </td>
                                ...
                                <td style='text-align: center;'>
                                    <?php
                                    if ($tok > 0) {
                                        $pperformance = $garmentur / ($tok) * 100;
                                    } else {
                                        $pperformance = 0;
                                    }
