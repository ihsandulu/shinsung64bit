...
<div class="row">
  <div class="col-md-12">
    ...
    $sql = $this->db->from("SGI_LEAN.dbo.v_schedule_produksi_2021_hari_ini")
      ...
      ->group_by("KANAAN_PO, STYLE_NO, ITEM")
    ...
    ?>
        ...
        <table class="table" width="100%;">
          <thead>
            <tr>
              ...
              <th width="70" align="center">HANGTAG</th>
          ...
          <tbody>
            ...
            foreach ($sql->result() as $row) { ?>
              <td><a href="<?= base_url("Qa_end_line/cekin/" . $line . "/" . $row->ID . "?KANAAN_PO=" . $row->KANAAN_PO . "&STYLE_NO=" . $row->STYLE_NO); ?>"><button type="button" class="btn btn-danger btn-md"> <i class="fa fa-sign-in"></i> </button></a></td>

              <td><a href="<?= base_url("Qa_end_line/cekout/" . $line . "/" . $row->ID . "?KANAAN_PO=" . $row->KANAAN_PO . "&STYLE_NO=" . $row->STYLE_NO); ?>"><button type="button" class="btn btn-warning btn-md"> <i class="fa fa-sign-in"></i> </button></a></td>

              <td><a href="<?= base_url("Qa_end_line/ironing/" . $line . "/" . $row->ID . "?KANAAN_PO=" . $row->KANAAN_PO . "&STYLE_NO=" . $row->STYLE_NO); ?>"><button type="button" class="btn btn-success btn-md"> <i class="fa fa-plug"></i> </button></a></td>

              <td><a href="<?= base_url("Qa_end_line/hangtag/" . $line . "/" . $row->ID . "?KANAAN_PO=" . $row->KANAAN_PO . "&STYLE_NO=" . $row->STYLE_NO); ?>"><button type="button" class="btn btn-info btn-md"> <i class="fa fa-tag"></i> </button></a></td>
              ...
              <td><a href="<?= base_url("Qa_end_line/upload_style_img/" . $line . "/" . $row->ID . "?KANAAN_PO=" . $row->KANAAN_PO . "&STYLE_NO=" . $row->STYLE_NO); ?>"><button type="button" class="btn btn-secondary btn-md"> <i class="fa fa-file-image-o"> </i> </button></a></td>
              <td><?= $row->KANAAN_PO; ?></td>
              <td><?= $row->STYLE_NO; ?></td>
              <td><?= $row->ITEM; ?></td>
              </tr>
      ...
      <div style="overflow-x:auto;"> <B> IF THERE IS A SCHEDULE THAT IS NOT ON THE LIST, PLEASE CONTACT PPIC SCHEDULE </B></div>
    <?php
    } else {
