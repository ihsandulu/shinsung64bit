...
<script language="javascript">
  $('#page_open').load('<?php echo base_url() . 'Qa_end_line_dashboardb/list_report_detailb?date1=' . $tanggal; ?>');
</script>
...
<script>
  ...
  function openModal(lines) {
    // alert("<?php echo base_url() . 'Qa_end_line_dashboardb/sp_dashboard_Qa_end_line_all_line_detail_defect_per_lineb/' ?>" + lines  +"?date1=<?= $tanggal; ?>");
    var modal = document.getElementById("myModal");
    modal.style.display = "block";
    document.getElementById("judul_modal").innerHTML = "DAFTAR DEFECT LINE : " + lines;
    $.ajax({
      type: 'POST',
      url: "<?php echo base_url() . 'Qa_end_line_dashboardb/sp_dashboard_Qa_end_line_all_line_detail_defect_per_lineb/' ?>" + lines + "?date1=<?= $tanggal; ?>",
      dataType: "JSON",
      data: {},
      success: function(response) { 
        ...
        defect_perjam(lines);
        // alert('<?php echo base_url() . '/Qa_end_line/display_inspect_dasbordb/' ?>' + lines + '?date1=<?= $tanggal; ?>');
        $('#display').load('<?php echo base_url() . '/Qa_end_line/display_inspect_dasbordb/' ?>' + lines + '?date1=<?= $tanggal; ?>');
      }
    });
  ...
  function renderTable_summary(data, nama_tabel) {
    ...
    var rows = '<tbody><tr><td width="7%" style="text-align:center;"><strong>CODE</strong></td><td width="54%"><strong>INFORMATION</strong></td><td width="22%"><strong>GARMENT</strong></td><td width="17%"><strong>PERCENTAGE</strong></td></tr></tbody>';
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      rows += "<td>" + data[i].persen_defect.toFixed(1) + " %</td>";
      rows += "</tr>"; */
    }

    for (var kode in data) {
        if (data.hasOwnProperty(kode)) {
            var name = data[kode].name;
            var count = data[kode].count;
            var persen = data[kode].persen;

            rows += "<tr>";
      rows += "<td>" + kode + "</td>";
      rows += "<td>" + name + "</td>";
      rows += "<td>" + count + "</td>";
      rows += "<td>" + persen.toFixed(1) + " %</td>";
      rows += "</tr>";

            /* console.log("Kode Defect: " + kode);

            // Mengakses count dan persen
            console.log("Count: " + count);
            console.log("Persen: " + persen);

            // Looping melalui unit-unit dalam kode defect
            for (var key in data[kode]) {
                if (data[kode].hasOwnProperty(key) && !isNaN(key)) {  // Mengecek jika key adalah index numerik
                    console.log("Unit Name: " + data[kode][key]);

                    // Misalnya, Anda ingin menambahkan data ke dalam modal
                    var content = document.getElementById("modalContent");
                    content.innerHTML += "<p>Kode Defect: " + kode + " - Unit: " + data[kode][key] + " (Count: " + count + ", Persen: " + persen + "%)</p>";
                }
            } */
        }
    }
  ...
  function defect_perjam(lines) {
    $.ajax({
      type: 'POST',
      url: "<?php echo base_url() . 'Qa_end_line/hasil_defect_perjamb/' ?>" + lines + "?date1=<?= $tanggal; ?>",
      dataType: "JSON",
      data: {
