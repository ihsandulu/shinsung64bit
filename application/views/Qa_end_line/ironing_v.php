...
<body>
            ...
            <div class="col-xs-7 col-sm-7 col-md-7 col-lg-7 colomn_satu m-0 p-0">
                ...
                <div class="container_form1" id="form_defect">
                    ...
                    <div class=" col-md-12 m-0 p-0 " id="tombold">
                        ...
                        <button class="button" style="background-color:darkgreen;" onClick="btnironing()">OK</button>
                ...
                <div class="row  m-0">
                    ...
                    <div class="col-md-3">
                        <div class="row kangka4"  style="">
                            <div class="tangka1 col-md-12 col-sm-12 col-xs-12 text-center"> SEWING</div>
    ...
    <script>
        function btnironing() {
            ...
            if (id_schedule != "" && QTY != "" && LINE != "") {
                // alert("<?= base_url("api/insertironing"); ?>?QTY="+QTY+"&LINE="+LINE+"&id_schedule="+id_schedule);
                $.get("<?= base_url("api/insertironing"); ?>", {
                        QTY: QTY,
                        LINE: LINE,
...
<script>
    ...
    function refresh_qty_cek() {
        ...
        if (id_schedule != "") {
            // alert("<?php echo base_url() . 'Qa_end_line/ironok/' . $line . '/'; ?>" + id_schedule + "/<?= date("Y-m-d"); ?>");
            $.ajax({
                type: 'POST',
                url: "<?php echo base_url() . 'Qa_end_line/ironok/' . $line . '/'; ?>" + id_schedule + "/<?= date("Y-m-d"); ?>",
                dataType: "JSON",
                data: {},
                success: function(response) {
                    ...
                    $("#jml_persen").html('0 % ');
                    var persen_ironing = 0;
                    if (qty_tchecking > 0) {
                        persen_ironing = qty_tchecking / qty_sewing * 100;
                    }
                    document.getElementById("jml_persen").innerHTML = persen_ironing.toFixed(1);
                    if (persen_ironing >= 100) {
    ...
    function total_output_perline() {
        var id_schedule = $("#id_schedule").val();
        // alert("<?php echo base_url() . 'api/hasillinejamironing'; ?>?tanggal=<?= date("Y-m-d"); ?>&line_sewing=<?= $line; ?>&"+id_schedule);
        if (id_schedule != "") {
            $.get("<?php echo base_url() . 'api/hasillinejamironing'; ?>", {
                    tanggal: '<?= date("Y-m-d"); ?>',
                    line_sewing: '<?= $line; ?>',
                    id_schedule: id_schedule
                })
