            ...
            <table border="1" width="100%" id="tabel" class="table table-striped table-bordered">
                ...
                <tbody>
                    ...
                    foreach ($report_array as $key => $report) {
                        ...
                        echo  "<td $kanan>" . number_format($report['qty_order'], 0, ".", ",") . "</td>";
                        $g_qty_order += $report['qty_order'];
                        echo  "<td $kanan>" . number_format($report['qty_check'], 0, ".", ",") . "</td>";
                        ...
                        foreach ($daftar_defect as $key => $defect_list) {
                            ...
                            if ($data) {
                                ...
                                echo "<td $kanan>" . number_format($data[0]['qty_defect'], 0, ".", ",")   . "</td>";
                            } else {
                                $sumdefect += 0;
                                echo "<td>  -  </td>";
                            }
                        }
                        echo  "<td $kanan>" . number_format($sumdefect, 0, ".", ",") . "</td>";
                        if ($qtycheck > 0) {
                            $pdr = $sumdefect / ($qtycheck+$sumdefect) * 100;
                        } else {
                            $pdr = 0;
                        }
                        echo  "<td $kanan>" . number_format($pdr, 1, ".", ",")   . "</td>";
                        echo "</tr>";
                    }
                    ?>
                </tbody>
                <tfoot>
                    <tr style="background-color : lightgray;">
                        ...
                        <td colspan="" style=' white-space: nowrap; text-align:right'><?php echo number_format($g_qty_order, 0, ".", ",") ?></td>
                        <td colspan="" style=' white-space: nowrap; text-align:right'><?php echo number_format($g_qty_cek, 0, ".", ",") ?></td>
                        <?php
                        $total_defect = 0;
                        foreach ($daftar_defect as $key => $defect_list) {
                            $data = filterArray_basedonline_kodedefect($sum_defect, str_replace('0', 'x', $defect_list['kode']));
                            if ($data) {
                        ...
                        ?>
                        <td colspan="" style=' white-space: nowrap; text-align:right'><?php echo number_format($total_defect, 0, ".", ","); ?></td>
                        <td colspan="" style=' white-space: nowrap; text-align:right'>
                            <?php
                            if ($g_qty_cek > 0) {
                                $tpdr = $total_defect / ($g_qty_cek+$total_defect) * 100;
                            } else {
                                $tpdr = 0;
                            }
