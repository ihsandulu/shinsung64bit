...
class Qa_end_line_dashboardb extends MY_Controller
{
    private $db_kis = "";
    ...
    <button type="submit" class="btn btn-default">Submit</button>
        ...
        $this->loadViews('Qa_end_line/list_reportb', $data);
    }
    ...
    public function sp_dashboard_Qa_end_line_all_line_detail_defect_per_lineb($line)
    {
        $date1 = $_GET["date1"];
        ...
        $def = $this->db
            ->from("inspect_v2")
            ->select("kode_defect, unit_name, count(unit_name)AS jmlun, MAX(keterangan)AS keterangan ")
            ->join("daftar_defect", "daftar_defect.kode=inspect_v2.kode_defect", "left")
            ->where("line", $line)
            ->where("kode_defect !=", "OK")
            ->where("unit_name IS NOT NULL")
            ->where("CONVERT(VARCHAR(10), time_stamp, 120)=", $date1)
            ->group_by("kode_defect, unit_name")
            ->order_by("kode_defect ASC, unit_name ASC")
            ->get();
        // echo $this->db->last_query();
        $data = array();
        foreach ($def->result() as $row) {
            $data[$row->kode_defect]["name"] = $row->keterangan;
            $data[$row->kode_defect]['count'] = isset($data[$row->kode_defect]['count']) ? $data[$row->kode_defect]['count'] + 1 : 1;
            $data[$row->kode_defect]['persen'] = $data[$row->kode_defect]['count'] / $jok * 100;
            $data[$row->kode_defect][] = $row->unit_name;
        }
    ...
    public function list_report_detailb()
    {
        $tanggal = date('Y-m-d');
        if (isset($_GET["date1"])) {
            $tanggal = $_GET["date1"];
    ...
    <button type="submit" class="btn btn-default">Submit</button>
        ...
        // $query = $this->db->query("exec [dashboard_Qa_end_line_all_line_hari_ini1] ?", array($tanggal));
        // $data['persen_defect'] = $query->result_array();
        $this->load->view('Qa_end_line/list_report_detailb', $data);
    }
